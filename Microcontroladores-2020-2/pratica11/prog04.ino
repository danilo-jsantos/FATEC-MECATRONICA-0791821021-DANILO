...
void  inicializar_wifi () {
  ...
  Wi-fi. modo (WIFI_STA);
  Wi-fi. começar ( " SUA_REDE " , " SUA_SENHA " );
...
void  pega_dado_com_arduinojson () {
  // Enviar pedido
  http. useHTTP10 ( verdadeiro );
  http. começar (URL_SERVIDOR);
  http. GET ();
  
  // Analisar a resposta
  DynamicJsonDocument doc ( 2048 );
  desserializeJson (doc, http. getStream ());
  
  // Ler valores
  Serial. println ( " Dados Recebidos: " );
  // char msg [100];
  // sprintf (msg, "CEP:% s \ n", doc ["cep"]. as <const char *> ());
  // Serial.print (msg);
  Serial. imprimir ( " CEP: " );
  Serial. println (doc [ " cep " ]. as < const  char *> ());
  Serial. imprimir ( " Logradouro: " );
